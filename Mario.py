...
def flamehitsmario(playerrect, flames):   
    for f in flame_list:                    #what is flame_list
        if playerrect.colliderect(f.imagerect): #what is colliderect
            return True
        return False
...
startimage = load_image('first.png')
startimagerect = startimage.get_rect()
startimagerect.centerx = window_width/2
startimagerect.centery = window_height/2-100
...
drawtext('PRESS ANY KEY TO PLAY', font, Canvas,(window_width/3 + 10), (window_height/3 + 200))
...
while True: #initializing a new game
    ...
    gameover.stop() #why are we writing this ? 
    pygame.mixer.music.play(-1,0.0) #why are we writing this ?
